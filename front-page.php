...
<section class="section" id="services">
	<div class="container">
		...
		<div class="services-grid" data-reveal-stagger>
			<?php for ( $i = 1; $i <= 6; $i++ ) : ?>
				<article class="service-card"><div class="icon-box <?php echo esc_attr( teznevise_mod( "svc{$i}_color" ) ); ?>"><i class="<?php echo esc_attr( teznevise_mod( "svc{$i}_icon" ) ); ?>" aria-hidden="true"></i></div><h3><?php echo esc_html( teznevise_mod( "svc{$i}_title" ) ); ?></h3><p><?php echo esc_html( teznevise_mod( "svc{$i}_text" ) ); ?></p><a class="link-arrow" href="<?php echo esc_url( teznevise_url( teznevise_mod( "svc{$i}_url" ) ) ); ?>"><?php esc_html_e( 'جزئیات خدمت', 'teznevise' ); ?></a></article>
			<?php endfor; ?>
		</div>
	</div>
</section>
