...
if ( empty( $menu_locations['mobile'] ) && ! empty( $menu_locations['primary'] ) ) {
	// Keep the full editor-managed Main hierarchy available in the drawer until
	// a dedicated Mobile Menu is assigned in WordPress.
	$menu_args['menu'] = (int) $menu_locations['primary'];
}
?>
<div id="mobile-navigation" class="mobile-nav" data-mobile-menu role="dialog" aria-modal="true" aria-label="<?php esc_attr_e( 'منوی موبایل', 'teznevise' ); ?>" hidden>
	<div class="mobile-nav-panel">
		...
		<nav class="mobile-nav-links" aria-label="<?php esc_attr_e( 'منوی موبایل', 'teznevise' ); ?>"><?php echo wp_nav_menu( $menu_args ); ?></nav>
