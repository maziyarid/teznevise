 ...
 * is never overwritten. Functional calculator/form shortcodes are preserved
...
function teznevise_wxr_import_classic_editor( $limit = -1, $offset = 0 ) {
	...
	foreach ( $pages as $page ) {
		...
		if ( function_exists( 'teznevise_page_has_editorial_copy' ) && teznevise_page_has_editorial_copy( $raw ) ) {
			++$result['skipped'];
			continue;
		}
...
function teznevise_maybe_import_classic_editor() {
	if ( ! is_admin() || ! current_user_can( 'edit_pages' ) || '1.9.6' === get_option( 'teznevise_classic_import_version' ) ) {
		return;
	}
	$batch_size = 10;
