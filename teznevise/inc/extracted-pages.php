 ...
 * are removed. A small threshold avoids treating an icon label as a page.
 *
...
function teznevise_prepare_classic_disclosure_html( $content, $post_id ) {
	...
	$content = preg_replace_callback( '/<\/?h1\b/iu', static function ( $match ) {
		return 0 === strpos( $match[0], '</' ) ? '</h2' : '<h2';
	}, (string) $content );
	$ids = array();
	$content = preg_replace_callback( '/\bid=([' . "'\"" . '])([^' . "'\"" . ']+)\1/iu', static function ( $match ) use ( $prefix, &$ids ) {
		$old         = sanitize_title( $match[2] );
		$ids[ $old ] = $prefix . $old;
		return 'id="' . esc_attr( $ids[ $old ] ) . '"';
	}, $content );
	foreach ( $ids as $old => $new ) {
		$content = preg_replace_callback( '/\b(href|aria-controls)=([' . "'\"" . '])#?' . preg_quote( $old, '/' ) . '\2/iu', static function ( $match ) use ( $new ) {
...
function teznevise_the_page_leftover_content( $post_id = 0 ) {
	...
	if ( ! teznevise_page_has_editorial_copy( $classic_raw ) ) {
		return;
	}
