...
/**
 * Initialize chat widget with REST API
 */
add_action(wp_footer, teznevise_init_chat_widget, 100);

function teznevise_init_chat_widget() {
    if (is_admin()) return;
    ...
    document.addEventListener(DOMContentLoaded, function() {
        ...
        if (typeof window.TezNeviseChat !== undefined) {
            window.TezNeviseChat.init({
                apiUrl: <?php echo wp_json_encode(rest_url(teznevise/v1)); ?>,
                siteUrl: <?php echo wp_json_encode(site_url()); ?>,
                nonce: <?php echo wp_json_encode(wp_create_nonce(teznevise-chat)); ?>,
            });
        }
        
        // Fallback: Fetch chat config via REST API
        fetch(<?php echo wp_json_encode(rest_url(teznevise/v1/chat-config)); ?>)
            .then(response => response.json())
            .then(data => {
                console.log(Chat widget initialized, data);
            })
            .catch(error => {
