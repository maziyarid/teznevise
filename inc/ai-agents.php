...
register_activation_hook( __FILE__, array( 'TezNevise_AI_Database', 'activate' ) );
register_deactivation_hook( __FILE__, array( 'TezNevise_AI_Database', 'deactivate' ) );
...
function teznevise_ai_shortcode( $atts ) {
	...
	return TezNevise_AI_Chat::render_chat(
		array(
			...
			'thinking_enabled'   => 'false' !== $atts['thinking_enabled'] && ( $tool['thinking_enabled'] ?? true ),
			'tool_config'        => $tool,
		)
	);
}
