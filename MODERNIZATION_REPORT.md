...
| React regression suite | **95 / 95** Node tests pass. |
...
Unicode emoji stripping no longer trips ESLint’s misleading-character-class rule. Five asynchronous loaders now use stable `useCallback` dependencies, the payment callback constructs its parameters inside the effect, and an obsolete lint suppression was removed. The PWA tests were also isolated from the local Teznevise Open Graph fixture so that generic metadata tests no longer depend on project-specific brand data.

The Vite development server now permits the managed preview subdomain pattern without disabling host protection globally. This fixes the prior blocked-host response and enables browser-based review through the authorized preview origin.

### WordPress Release and Quality Automation

The release check previously scanned `react-app/node_modules`, making it slow and inappropriate for source validation. It now excludes dependencies and build artifacts, derives the theme version from the canonical header, verifies that `functions.php`, `README.md`, and `readme.txt` match it, and checks the real Font Awesome asset rather than a nonexistent logo file. A false-positive secret rule was narrowed to the actual high-risk condition: committed private-key material.
...
| `react-app/src/lib/utils.ts` | Unicode-safe emoji/symbol stripping. |
